...
</dom-module>`;document.head.appendChild($_documentContainer$3.content);Polymer({is:"paper-button",behaviors:[PaperButtonBehavior],properties:{raised:{type:Boolean,reflectToAttribute:!0,value:!1,observer:"_calculateElevation"}},_calculateElevation:function(){if(!this.raised){this._setElevation(0)}else{PaperButtonBehaviorImpl._calculateElevation.apply(this)}}});const $_documentContainer$4=document.createElement("template");$_documentContainer$4.setAttribute("style","display: none;");$_documentContainer$4.innerHTML=`<custom-style>
  <style is="custom-style">
    html {
...
</custom-style>`;document.head.appendChild($_documentContainer$5.content);const $_documentContainer$6=document.createElement("template");$_documentContainer$6.setAttribute("style","display: none;");$_documentContainer$6.innerHTML=`<dom-module id="paper-icon-button">
  <template strip-whitespace="">
    <style>
      :host {
        display: inline-block;
...
</custom-style>`;document.head.appendChild($_documentContainer$7.content);Polymer({_template:html`
    <style>
      :host {
        display: inline-block;
        float: right;
...
</custom-style>`;document.head.appendChild($_documentContainer$8.content);Polymer({_template:html`
    <style>
      :host {
        display: block;
        padding: 8px 0;
...
<style>
...
</style>`;var anywhereStyles={AnywhereStyles:AnywhereStyles};class AnyQuickSearchBar extends PolymerElement{constructor(){super()}static get properties(){return{headerToggle:{type:String,value:"block",notify:!0},inputToggle:{type:String,value:"hidden",notify:!0},inputOn:{type:Boolean,value:!1}}}static get template(){return html`
      ${AnywhereStyles}
      <style>
        :host {
          flex: 1 auto;
    ...
    `}_onSearchTermChanged(evt){if(2<evt.detail.value.length){this.dispatchEvent(new CustomEvent("searchTermChanged",{detail:{inputOn:this.inputOn,searchTerm:evt.detail.value}}))}}_toogleSearchBar(){if("block"==this.headerToggle){this.headerToggle="hidden";this.inputToggle="block";this.inputOn=!0;this.$.paperInput.value="";this.$.paperInput.focus()}else{this.headerToggle="block";this.inputToggle="hidden";this.inputOn=!1}this.dispatchEvent(new CustomEvent("searchTermChanged",{detail:{inputOn:this.inputOn,searchTerm:this.$.paperInput.value}}))}}window.customElements.define("any-quick-search-bar",AnyQuickSearchBar);setPassiveTouchGestures(!0);setRootPath(AnyAppGlobals.rootPath);class AnyApp extends PolymerElement{constructor(){super();AnyAppGlobals.dbBinder=new PouchdbBinder;AnyAppGlobals.dbBinder._registerDbListener(AnyAppGlobals.anydb);let firstTimeLoad=!0,bluemixData={name:"anywhere",username:"d2a67c57-1c89-4ae9-a71c-c18ca298f338-bluemix",password:"e67deccd52bfda83dc235b8346d58949e6e612aa32086d5c249a53d0bfb79f22",host:"d2a67c57-1c89-4ae9-a71c-c18ca298f338-bluemix.cloudant.com",port:443,url:"https://d2a67c57-1c89-4ae9-a71c-c18ca298f338-bluemix:e67deccd52bfda83dc235b8346d58949e6e612aa32086d5c249a53d0bfb79f22@d2a67c57-1c89-4ae9-a71c-c18ca298f338-bluemix.cloudant.com"},remoteDbUrl=`https://${bluemixData.username}:${bluemixData.password}@${bluemixData.host}:${bluemixData.port}/anywhere`;PouchDB.sync("anywhere",remoteDbUrl,{live:!0,retry:!0}).on("change",()=>{if(firstTimeLoad){console.log("Finish first contact with database");this.$.listView.requestLoadWorkorders();firstTimeLoad=!1}});this.firstLoad=!1}static get properties(){return{pageTitle:{type:String,value:"My Assigned Work"},page:{type:String,reflectToAttribute:!0,observer:"_pageChanged"},firstLoad:{type:Boolean,value:!0},routeData:Object,subroute:Object}}static get observers(){return["_routePageChanged(routeData.page)"]}static get template(){return html`
      ${AnywhereStyles}
      <style>
        :host {
