define(["./any-app.js"],function(_anyApp){"use strict";class AnyListItem extends _anyApp.PolymerElement{constructor(){super();(0,_anyApp.afterNextRender)(this,function(){this.addEventListener("click",this._onClick)})}static get properties(){return{item:{type:Object,value:null,reflectToAttribute:!0,notify:!0,readonly:!1,observer:"_dataChanged"},tabindex:{type:Number,value:-1},statusClass:{type:String,value:""},timerIcon:{type:Object,value:null}}}static get template(){return _anyApp.html`
        ...
        <div class="flex flex--row flex--justify-end flex--align-center">
          ...
          <div>
            ...
            <template is="dom-if" if="[[_isTimer(item.timer, 'on')]]">
              <button class="button button__icon" aria-label="Start timer">${_anyApp.timerOn}</button>
            </template>
            <template is="dom-if" if="[[_isTimer(item.timer, 'off')]]">
              <button class="button button__icon" aria-label="Stop timer">${_anyApp.timerOff}</button>
            </template>
          </div>
